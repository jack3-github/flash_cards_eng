...
// 答えを表示し、かつ読み上げる関数 (旧 showAnswer() を変更)
...
function speakText(text) {
    ...
    // 必要に応じて、ここでutterance.lang = 'en-US'; を追加することもできます。
    // その場合、以下のfind条件と一致している必要があります。
    utterance.lang = 'en-US'; // 明示的に言語を設定

    const englishVoice = voices.find(voice => voice.lang === 'en-US' || voice.lang.startsWith('en-'));
    if (englishVoice) {
        utterance.voice = englishVoice;
    } else {
        // 適切な英語ボイスが見つからない場合、利用可能な最初のボイスを使用
...
// 「答えを見る」ボタンがクリックされたら、答えを表示して読み上げる (showAnswer から showAnswerAndSpeak へ変更)
